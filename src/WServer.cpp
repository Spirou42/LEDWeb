...
typedef enum _messageType { RGBMessage = 1, RequestMessage = 2 } MessageType_t;
...
void processRGBMessage(JsonObject &color) {
  ...
  Serial << "ColorMessage: (R:" << (int)(color["red"])
         << " ,G:" << (int)(color["green"]) << " ,B:" << (int)(color["blue"])
         << ")" << endl;
...
void processEffectMessage(JsonObject &effect) {
  ...
  FastLEDAddOns::currentRunningEffect =
      FastLEDAddOns::systemEffectList.begin() + (int)(effect["Effect"]);
}
...
void processEffectParameterMessage(JsonObject &parameter) {
  ...
  Serial << "EffectParameter: " << name << "(" << index << ") to " << value
         << endl;
  ValueWrapper *currentWrapper =
      (*FastLEDAddOns::currentRunningEffect)->parameterAt(index);
  currentWrapper->setValue(value);
}

/**
 * @brief process the WS message from a the client, extract json and dispatch
 * according to type
 *
 * @param data
 */
...
/**
 * @brief websocket event handler, handles connections, error and data
 * collection dispatches compleately received messages to the client message
 * processor
 *
 * @param server
 * @param client
...
void onWebSocketEvent(AsyncWebSocket *server, AsyncWebSocketClient *client,
  ...
  if (type == WS_EVT_CONNECT) {
    ...
    if (info->final && info->index == 0 && info->len == len) {
      ...
      // printf("ws[%s][%u] %s-message[%llu]: ", server->url(), client->id(),
      ...
      printf("ws[%s][%u] frame[%u] %s[%llu - %llu]: ", server->url(),
             client->id(), info->num,
...
/**
 * @brief connection handler for the event source, sends current system data to
 * the client after connection
 *
 * @param client
 */
...
/**
 * @brief template variable processor of the Webserver currently knows about
 * HOSTNAME and CURRENTCOLOR
 *
 * @param var
 * @return String
 */
...
void handleRoot(AsyncWebServerRequest *request) {
  ...
  if (request->hasParam("nextColor")) {
    ...
    // Serial << "Got a new color "<<request->getParam("nextColor")->value()
    // <<endl;
