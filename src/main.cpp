...
void dumpParameters(Effect *currentEffect) {
  ...
  for (size_t k = 0; k < currentEffect->numberOfParameters(); k++) {
    ...
    if (vw) {
      Serial << vw->getValue() << "\t(" << vw->getMinValue() << ", "
             << vw->getMaxValue() << ")" << endl;
    } else {
      Serial << endl;
    }
...
void postFrameCallback(unsigned long now) {
  ...
  if (lastPalette != currentSystemPalette) {
    ...
    Serial << "Palette " << (currentSystemPalette - systemPalettes.begin())
           << endl;
...
void setup() {
  ...
  scanNetworks();
  if (WiFi.isConnected()) {
    Serial << "Connected as " << HOSTNAME << ".local (" << WiFi.localIP()
           << ") to " << WiFi.SSID() << endl;
    startServices();
  } else {
...
void loop() {
  ...
  EVERY_N_MINUTES(10) {
    ...
    if (FastLEDAddOns::currentRunningEffect ==
        FastLEDAddOns::systemEffectList.end()) {
      FastLEDAddOns::currentRunningEffect =
          FastLEDAddOns::systemEffectList.begin();
    }
  }
  EVERY_N_MINUTES(3) {
