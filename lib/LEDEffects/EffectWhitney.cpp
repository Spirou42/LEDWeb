...
sequence
Q0Z1U180R400V1&20 #r400 200 20 #z1 5 60 %r #r200 30 60%r #z5 14 60 #r30 75 30
%z#z14 1 60#r75 400 60 %z Q4Z1U180R400V1&20 #r400 200 20 #z1 5 60 %r #r200 30
60%r #z5 14 60 #r30 75 30 %z#z14 1 60#r75 400 60 %z
...
void EffectWhitney::startEffect() {
  ...
  if (_parameters.empty()) {
    ValueWrapper *k =
        new ValueWrapper(&_lumenScale, 0, 400, "Luma Scale", "LSC");
    addParameter(k);
...
void EffectWhitney::frame(unsigned long now) {
  ...
  // Serial <<clearLineRight<<"W:"<<display.displayWidth()<<"
  // h:"<<display.displayHeight()<<endl;
  ...
  for (uint16_t i = 0; i < numberOfPixels; i++) {
    ...
    float hue =
        (256.0 * 0.001 - hueStep) /*  hueSpeed)*/;  //(r /*+ hueStep*/);//
    ...
    // if(i==0){
    //   Serial << ScreenPos(15,1)<<clearLineRight<<" hStep:"<<hueStep<<"
    //   ("<<hSpeedT<<")"<<endl; if (hSpeedT != 0.0) {
    //     Serial << "Ba "<<(1-hSpeedT)<<endl;
    //   }
    //   Serial.print(hSpeedT);
    // }
    CRGB c =
        ColorFromPalette((*currentSystemPalette)->second.palette, globalHue);
    CHSV hC = rgb2hsv(c);
    // uint8_t k = lum*255;
    //  while(k>255){
    //    k-=255;
