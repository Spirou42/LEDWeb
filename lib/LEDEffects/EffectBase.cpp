 ...
 * @brief common and base structures for running effects. Defines global
 * parameters, paletes and mapping for the LED matrix as defined in
 * LasLEDAddOns.
 * @version 0.1
 * @date 2021-09-29
 *
...
ValueWrapper ledBrightnessW(&ledBrightness, 1, 255, "Brightness", "LB",
                            "System");
ValueWrapper currentFrameRateW(&currentFrameRate, 1, 5000, "Frame Rate", "FR",
                               "System");

ValueWrapper hueStepW(&globalHueStep, 0, 50, "HUE Step", "HS", "Palettes");
ValueWrapper hueFrameDelayW(&globalHueFrameDelay, 1, 500, "HUE Delay", "HFD",
                            "Palettes");
...
void initialiseLEDs() {
  FastLED
      .addLeds<LED_TYPE, DATA_PIN, CLOCK_PIN, COLOR_ORDER, DATA_RATE_MHZ(1)>(
          leds, NUM_LEDS)
...
PaletteList initializeSystemPalettes() {
  ...
  tmp.push_back(new PalettePair("Rainbow Stripes",
                                Palette_t(RainbowStripeColors_p, false)));
  ...
  tmp.push_back(
      new PalettePair("Temperature", Palette_t(temperature_gp, false)));
  ...
  tmp.push_back(
      new PalettePair("Yellow Sunset", Palette_t(Sunset_Yellow_gp, false)));
  return tmp;
}
...
EffectDrops dropEffect = EffectDrops();
...
EffectList initializeSystemEffects() {
  ...
  tmp.push_back(&dropEffect);
  return tmp;
}

EffectList FastLEDAddOns::systemEffectList = initializeSystemEffects();
EffectList::iterator FastLEDAddOns::currentRunningEffect =
    systemEffectList.begin();
...
FastLEDAddOns::ParameterList systemParameterList =
...
void startupEffects(avrQueue *queue, postFrame callback) {
  ...
  queue->scheduleFunction(FastLEDAddOns::effectRunner, (void *)callback, "EFCT",
                          0, 1000 / 60);
}
