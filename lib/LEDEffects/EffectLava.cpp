...
void EffectLava::applyHeat() {
  ...
  for (uint8_t x = 0; x < MATRIX_WIDTH; x++) {
    for (uint8_t y = 1; y < MATRIX_HEIGHT; y++) {
      int16_t q = heatLosBase + (tempMap[x][y - 1] - tempMap[x][y]) /
                                    3; // + (tempMap[x][y-1])/10;
...
void EffectLava::mapHeatToBackground() {
  for (uint8_t x = 0; x < MATRIX_WIDTH; x++) {
    for (uint8_t y = 0; y < MATRIX_HEIGHT; y++) {
      CRGB color = ColorFromPalette((*currentSystemPalette)->second.palette,
                                    tempMap[x][y]);
...
void EffectLava::frame(unsigned long now) {
  ...
  ledMatrix.circle(FLPoint(18, y), radius, CRGB::Orange, true);
  ledMatrix.circle(FLPoint(18, y), radius, CRGB::DarkRed, false);
  int oldY = y;
  y = t * p;
  if (y < MATRIX_HEIGHT) {
    t++;
